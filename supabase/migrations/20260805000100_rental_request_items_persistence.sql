...
create table if not exists private.rental_equipment_catalog (
  ...
  constraint rental_equipment_catalog_category_check
    check (category in ('Heavy Equipment', 'Trailers', 'Tools', 'Motorcycles')),
...
values
  ('bobcat-t550-skid-steer', '2024 Bobcat T550 Track Loader', 'Heavy Equipment', 'active', true, 120, true, 'B57T133070', true, true),
  ('bobcat-e35r2-compact-excavator', '2025 Bobcat E35 Compact Excavator', 'Heavy Equipment', 'active', true, 275, true, 'B57920400', false, false),
  ('lamar-telescopic-dump-9-ton', '2025 Lamar Telescopic Dump 9 Ton Trailer', 'Trailers', 'active', true, 175, false, null, false, false),
  ('rawmax-tilt-deck-22', '2025 RawMax Tilt Deck 22’', 'Trailers', 'archived', false, 150, false, null, false, false),
  ('rawmaxx-dtx-24-tilt-deck', 'RawMaxx DTX 24’ Tilt Deck Trailer', 'Trailers', 'active', true, 150, false, null, false, false),
  ('utility-trailer', 'Utility Trailer', 'Trailers', 'archived', false, 100, false, null, false, false),
  ('scissor-lift', 'Electric Scissor Lift', 'Heavy Equipment', 'active', true, 140, false, null, false, false),
  ('hercules-sds-max-demolition-hammer', 'Hercules SDS-MAX Demolition Hammer', 'Tools', 'active', true, 50, false, null, false, false),
  ('bauer-65j-demolition-hammer', 'Bauer 65J Demolition Hammer', 'Tools', 'active', true, 50, false, null, false, false),
  ('plate-compactor', 'Central Machinery Plate Compactor', 'Tools', 'active', true, 60, false, null, false, false),
  ('wacker-rd12-roller', 'Wacker Neuson RD12 Roller', 'Heavy Equipment', 'active', true, 180, true, 'WNCRD12AEPUM06214', true, true),
  ('kobalt-hand-tamper', 'Kobalt 10×10 Hand Tamper', 'Tools', 'active', true, 10, false, null, false, false),
  ('harley-road-glide-2003', '2003 Harley Davidson Road Glide', 'Motorcycles', 'active', true, 180, false, null, false, false),
  ('harley-fld-switchback-2012', '2012 Harley Davidson FLD Switchback', 'Motorcycles', 'active', true, 130, false, null, false, false)
...
alter table public.rental_request_items
  drop constraint if exists rental_request_items_rental_request_fk;
alter table public.rental_request_items
  add constraint rental_request_items_rental_request_fk
  foreign key (rental_request_id)
  references public.rental_requests(id)
  on update restrict
  on delete restrict;
...
begin
  ...
  unsupported_fields := request_payload - array[
    'customer_type', 'business_name', 'full_name', 'phone', 'email',
    'fulfillment_type', 'project_type', 'notes', 'agreement_accepted'
  ]::text[];
  ...
  if (request_payload ->> 'agreement_accepted')::boolean is not true then
    raise exception using errcode = '22023',
...
begin
  ...
  loop
    ...
    unsupported_fields := item_payload - array[
      'equipment_id', 'start_date', 'end_date', 'quantity', 'notes'
    ]::text[];
...
revoke all on function private.is_feature_enabled(text)
  from public, anon, authenticated;
revoke all on function private.rental_request_item_editability(uuid)
  from public, anon, authenticated;
revoke all on function private.validate_rental_request_payload(jsonb)
  from public, anon, authenticated;
